...
class Calculator:
    def __init__(self, master):
        ...
        buttons = [
            ...
            ("0", lambda: self.append("0")), (".", lambda: self.append(".")), ("(", lambda: self.append("("), self.rbracket), (")", lambda: self.append(")"), self.lbracket),
            ("x²", self.square)
        ]
    ...
    def append(self, char):
        if self.formula == "0" and not char.isdigit():
            ...
            if self.formula and self.formula[-1] == ')':
                if self.formula[-1:] in ['+-', '-*', '/-', '+*', '*+', '/+', '*-', '*+', '/*', '/+', '**2']:
                    pass
                elif len(self.formula) > 1 and self.formula[-1] not in ['+', '-', '*', '/', '(', '**2']:
                    return
