...
class Calculator:
    def __init__(self, master):
        ...
        buttons = [
            ("±", self.change_sign), ("%", self.percent), ("C", self.clear), ("⌫", self.delete),
            ("x!", self.factorial), ("x²", self.square), ("√x", self.sqrt), ("÷", lambda: self.append("/")),
    ...
    def append(self, char):
        ...
        if re.search(r"00", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"01", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"02", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"03", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"04", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"05", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"06", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"07", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"08", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        elif re.search(r"09", self.formula) and not '.' in self.formula:
            self.formula = self.formula[:-1]
        ...
        try:
            if self.formula:
                ...
                if match:
                    ...
                    if self.formula[0] in ["-"] and values == "False":
                        self.formula = self.formula[1]
                        values = "True"
                    ...
                    self.formula = self.formula[:-len(num_str) - (2 * in_paren)] + replacement
    ...
    def percent(self):
        if self.formula[0] in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]:
            self.formula = str(eval(self.formula + "/" + "100"))
            self.locked = True
        elif self.formula[0] in "0":
            self.formula = "0"
            self.locked = False
        self.consecutive_digits = self.count_digits_before_operator(self.formula)
        self.update_display(self.formula)
    ...
    def factorial(self):
        if self.formula[0] in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]:
            result = math.factorial(int(self.formula))
            result_str = str(result)
            if len(result_str) > 18:
                self.update_display("Result too long")
                return
            ...
            self.formula = "0"
            self.locked = False
        elif self.formula[1] in "." or self.formula[2] in "." or self.formula[3] in "." or self.formula[4] in "." or self.formula[5] in "." or self.formula[6] in ".":
            self.formula = "0"
            self.locked = False
        else:
