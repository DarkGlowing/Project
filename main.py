    ...
    def modul(self):
        try:
            ...
            if match:
                ...
                num = match.group(3)
                if num and minus and not first_skobka and second_skobka:
                    print("hello")
                ...
                elif num and minus and first_skobka and second_skobka and not self.formula[-3] in "(":
                    if self.formula[-2] in num and self.formula[-3] in '-':
                        self.formula = self.formula[:-1] and self.formula[:-2] and self.formula[:-3]
                    self.formula = self.formula + "+" + num + ")"
                elif self.formula[-1] in ")" and self.formula[-2] in num and self.formula[-3] in "(":
                    print("hi")
                    if self.formula[-2] in num and self.formula[-3] in '-':
                        self.formula = self.formula[:-1] and self.formula[:-2] and self.formula[:-3]
                    self.formula = self.formula + num + ")"
