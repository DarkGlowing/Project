    ...
    def square(self):
        try:
            ...
            while i >= 0 and self.formula[i].isdigit() or self.formula[i] == '.':
                last_number = self.formula[i] + last_number
                i -= 1
            if last_number.endswith(".") and last_number.startswith("."):
                last_number = "0."
            if last_number == '0' or (last_number.startswith(".") and last_number.rstrip(".") == "0"):
                return
            else:
                self.formula += '**2'
                self.last_input_was_operator = True
