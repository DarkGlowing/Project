...
class Calculator:
    def __init__(self, master):
        ...
        buttons = [
            ...
            ("7", self.seven), ("8", self.eight), ("9", self.nine), ("×", lambda: self.append("*")),
            ("4", self.four), ("5", self.five), ("6", self.six), ("-", lambda: self.append("-")),
            ("1", self.one), ("2", self.two), ("3", self.three), ("+", lambda: self.append("+")),
    ...
    def zeros_confusion(self):
        operators = ["+", "-", "/", "*"]
        self.formulas = self.formula[1] or self.formula[2] or self.formula[3] or self.formula[4] or self.formula[5]
        if not re.search(r"0", self.formula):
            self.append("0")
        elif re.search(r"0", self.formula) and re.search(operators, self.formulas):
            self.append("0")
    ...
    def one(self):
        ...
        self.append("1")
        try:
            if self.formula[2] in ["0"] and self.formula[3] in numb:
                self.formula = self.formula[:3]
            elif self.formula[3] in ["0"] and self.formula[4] in numb:
                self.formula = self.formula[:4]
            elif self.formula[4] in ["0"] and self.formula[5] in numb:
                self.formula = self.formula[:5]
            elif self.formula[5] in ["0"] and self.formula[6] in numb:
                self.formula = self.formula[:6]
            elif self.formula[6] in ["0"] and self.formula[7] in numb:
                self.formula = self.formula[:7]
            elif self.formula[7] in ["0"] and self.formula[8] in numb:
                self.formula = self.formula[:8]
            elif self.formula[8] in ["0"] and self.formula[9] in numb:
                self.formula = self.formula[:9]
            elif self.formula[9] in ["0"] and self.formula[10] in numb:
                self.formula = self.formula[:10]
            elif self.formula[10] in ["0"] and self.formula[11] in numb:
                self.formula = self.formula[:11]
            elif self.formula[11] in ["0"] and self.formula[12] in numb:
                self.formula = self.formula[:12]
            elif self.formula[12] in ["0"] and self.formula[13] in numb:
                self.formula = self.formula[:13]
            elif self.formula[13] in ["0"] and self.formula[14] in numb:
                self.formula = self.formula[:14]
            elif self.formula[14] in ["0"] and self.formula[15] in numb:
                self.formula = self.formula[:15]
            elif self.formula[15] in ["0"] and self.formula[16] in numb:
                self.formula = self.formula[:16]
            elif self.formula[16] in ["0"] and self.formula[17] in numb:
                self.formula = self.formula[:17]
            elif self.formula[17] in ["0"] and self.formula[18] in numb:
                self.formula = self.formula[:18]
            elif self.formula[18] in ["0"] and self.formula[19] in numb:
